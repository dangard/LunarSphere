...
--		Lunar.API:Print("  itemTableNames[index] = " .. itemTableNames[index] .. "  " .. index);
...
--	local index, itemSpell, itemSpllIdLen;

--	itemSpllIdLen = 0;
	
	-- Croq Update
--	if not itemSpellID[itemCatagory] == nil then
--		itemSpllIdLen = string.len(itemSpellID[itemCatagory]);
--	end
	
--	for index = 1, (itemSpllIdLen / 6) do 
...
--									Lunar.API:Print("Seahorse Index " .. Lunar_Seahorse);
								else
...
-- Mount Stuff is Here.
		...
		if (itemType == "mount") then
			...
			local Lunar_areaId = GetCurrentMapAreaID();
...
--				Lunar.API:Print("Lunar_areaId - " .. Lunar_areaId);

			    if (((Lunar_areaId == 610) or (Lunar_areaId == 614) or (Lunar_areaId == 615)) and Lunar_Seahorse ~= 0) then 
			...
			if (canFly and ((Lunar_areaId ~= 610) and (Lunar_areaId ~= 614) and (Lunar_areaId ~= 615))) then
...
--							if (index > 10) then
--								Lunar.Button:AssignByType(_G["LunarSub" .. index .. "Button"], clickType, buttonType, stance);
--							else
--								Lunar.Button:AssignByType(_G["LunarMenu" .. index .. "Button"], clickType, buttonType, stance);
--							end
--						end
--
...
--	for index = 1, 10 do 
--		_G["LunarMenu" .. index .. "ButtonHeader"]:SetAttribute("state-state", _G["LunarMenu" .. index .. "ButtonHeader"]:GetAttribute("state"));
--	end
...
--	Lunar.API:Print("Number of Mounts " .. GetNumCompanions("mount"));
...
--Lunar.API:Print("  Itemspell = " .. itemSpell .. "   " .. itemID);
